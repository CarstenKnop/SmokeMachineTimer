...
void WiFiService::setupRoutes() {
  server.on("/", [this](){
    String html = F("<html><head><meta name='viewport' content='width=device-width,initial-scale=1'><title>SmokeMachineTimer</title><style>body{font-family:sans-serif;margin:10px;}h1{font-size:1.2em;}label{display:block;margin-top:8px;}footer{margin-top:16px;font-size:0.7em;color:#666;}button{font-size:1em;padding:4px 10px;margin-top:8px;}</style></head><body><h1>Smoke Machine Timer</h1>");
  ...
  auto portalHtml = F("<html><head><meta name='viewport' content='width=device-width,initial-scale=1'><title>SmokeTimer Portal</title></head><body><h2>SmokeMachineTimer</h2><p>Device captive portal.</p><p><a href='/' style='font-size:1.2em'>Open Control Page</a></p></body></html>");
  ...
  server.on("/ncsi.txt", [this](){ if(Serial) Serial.println(F("Captive: /ncsi.txt")); server.send(200,F("text/plain"),F("SmokeTimer")); }); // Windows expects a specific text, but any differing triggers portal
  server.on("/connecttest.txt", [this](){ if(Serial) Serial.println(F("Captive: /connecttest.txt")); server.send(200,F("text/plain"),F("SmokeTimer")); });
  server.on("/redirect", [this, portalHtml](){ if(Serial) Serial.println(F("Captive: /redirect")); server.send(200,F("text/html"),portalHtml); });
  server.on("/success.txt", [this](){ if(Serial) Serial.println(F("Captive: /success.txt")); server.send(200,F("text/plain"),F("SmokeTimer")); });
  ...
  server.onNotFound([this](){
    ...
    String html = F("<html><head><meta name='viewport' content='width=device-width,initial-scale=1'><title>Portal</title></head><body><h2>SmokeMachineTimer Portal</h2><p>This network is captive. <a href='/' style='font-size:1.1em'>Control Page</a></p></body></html>");
    server.sendHeader(F("Cache-Control"), F("no-store, no-cache, must-revalidate, max-age=0"));
    server.sendHeader(F("Pragma"), F("no-cache"));
    server.sendHeader(F("Expires"), F("0"));
    server.send(200, F("text/html"), html);
