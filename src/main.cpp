...
void printTimerValue(uint32_t value, int y, const char* label, int startX = 26) {
  char buf[8];
  ...
  int labelX = startX + 11 * (DIGITS + 1) + 2 + 8; // 11 is digit width for size 2
  display.setTextSize(1);
  display.setCursor(labelX, y + 7); // y+4 for vertical alignment
  display.print(label);
  display.setTextSize(2); // restore size for next use
}
...
void loop() {
  ...
  printTimerValue(offTime, 0, "OFF");

  // T2 row (second line, label ON)
  printTimerValue(onTime, 24, "ON");
